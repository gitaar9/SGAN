...
gt_image_2 = Image.open("{}_2.{}".format(*opt.image_path.split("."))).convert('RGB')
...
gt_image_2 = transform(gt_image_2).to(device).unsqueeze(0)

gt_images = torch.cat((gt_image, gt_image_2), dim=0)
...
if "halfcontainership" in opt.image_path:
    ...
    image_h_2 = 1.7824177233974483  # halfcontainer 3
    image_v_2 = 1.1590632341951803  # halfcontainer 3
    ...
    image_h_2 = -2.235022636602552  # Speedboat 4
    image_v_2 = 0.3589346441951802  # Speedboat 4
...
for i in range(n_iterations):
    ...
    loss = torch.nn.L1Loss()(both_frames, gt_images)
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                img, _ = generator.staged_forward_with_frequencies(w_frequencies + w_frequency_offsets,
                                                                   ...
                                                                   h_mean=image_h, max_batch_size=opt.max_batch_size,
                                                                   lock_view_dependence=lock_view_dependence,
                                                                   **render_options)
...
with torch.no_grad():
    for pitch, yaw in tqdm(trajectory):
        ...
        with torch.cuda.amp.autocast():
            frame, depth_map = generator.staged_forward_with_frequencies(w_frequencies + w_frequency_offsets,
                                                                         w_phase_shifts + w_phase_shift_offsets,
                                                                         max_batch_size=opt.max_batch_size,
                                                                         lock_view_dependence=lock_view_dependence,
                                                                         **render_options)
...
writer = skvideo.io.FFmpegWriter(os.path.join(f'{opt.output_dir}', output_name),
