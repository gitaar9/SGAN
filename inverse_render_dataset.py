...
def render_folder(generator, folder_path, image_size, output_dir, options, render_options, max_batch_size,
    ...
    print(f'Optimizing for {folder_path}')
    _, found_frequencies, found_phase_shifts = find_latent_z(
        generator=generator,
        n_iterations=700,
        gt_images=gt_images,
    ...
    output_path = os.path.join(output_dir, folder_path.split('/')[-1])
    Path(output_path).mkdir(parents=True, exist_ok=True)

    print(f'Generating for {folder_path}')
